# EsTodo

待办事项 + 番茄钟应用

## 📦 快速下载

从 [Releases](https://github.com/你的用户名/EsTodo/releases) 页面下载最新版本：

- **Windows**: `EsTodo-windows.zip` - 解压后直接运行 `EsTodo.exe`
- **Linux**: `EsTodo-linux.tar.gz` - 解压后运行 `EsTodo`

## 功能特性

- ✅ 待办事项管理（增删改查）
- ✅ 子待办支持（无限层级）
- ✅ Markdown 内容
- ✅ 优先级标记
- ✅ 深色/浅色主题切换
- ✅ 番茄钟（工作/短休息/长休息）
- ✅ 番茄钟暂停/继续/停止
- ✅ 系统通知（plyer）
- ✅ 番茄钟与待办关联
- ✅ 番茄日历热力图（GitHub 风格）
- ✅ 点击日期查看当天番茄钟详情
- ✅ 标签系统（新建/编辑/删除标签，待办可关联标签）
- ✅ JSON 导入导出（备份/迁移数据）
- ✅ PyInstaller 打包（Windows/Linux 单文件 exe）
- ✅ GitHub Actions CI/CD（自动构建 Release）
- ✅ 待办事项归档功能（根待办及其子待办一起归档）
- ✅ 独立归档页面查看已归档事项
- ✅ 只读查看模式（双击待办查看，右键编辑）
- ✅ 待办列表右侧复选框快速切换完成状态
- ✅ 刷新列表保持展开/选中状态
- ✅ 系统托盘图标（最小化到托盘）
- ✅ 关闭窗口行为选择（询问/最小化/退出）
- ✅ 完整设置页面（侧边栏分类）
  - 番茄钟设置（工作/休息时长）
  - 外观设置（主题切换）
  - 系统设置（关闭行为、最小化到托盘）
  - 快捷键设置（显示）
- ✅ 设置持久化到数据库

## 安装

### 环境要求

- Python 3.12+
- conda

### 开发环境设置

```bash
# 激活虚拟环境
conda activate estodo

# 安装依赖
pip install -r requirements.txt
```

## 运行

```bash
# 激活环境
conda activate estodo

# 运行应用
python -m estodo.main
```

或者直接运行：

```bash
python src/estodo/main.py
```

## 使用说明

### 系统托盘
- 应用启动后会在系统托盘显示图标
- **双击托盘图标**：显示/隐藏主窗口
- **右键托盘图标**：打开菜单
  - 显示/隐藏：切换主窗口可见性
  - 新建待办：快速打开新建待办界面
  - 启动番茄钟：跳转到番茄钟页面
  - 退出：完全关闭应用

### 窗口行为
- **最小化窗口**：默认最小化到托盘（可在设置中关闭）
- **关闭窗口**：
  - 默认弹出选择对话框
  - 可选择"最小化到托盘"或"退出应用"
  - 可勾选"记住我的选择"

### 待办事项交互

- **单击待办**：只选中，不显示详情
- **双击待办**：打开只读查看界面
- **右键菜单 → 编辑**：打开编辑界面
- **右侧复选框**：快速切换完成状态（活跃待办列表）

### 归档功能

- **归档待办**：右键根待办 → 选择"📦 归档"（根待办及其所有子待办会一起归档）
- **查看归档**：左侧导航点击"📦 归档事项"
- **取消归档**：在归档页面右键待办 → 选择"📦 取消归档"

### 系统托盘与关闭行为

- **最小化到托盘**：点击窗口最小化按钮或关闭时选择"最小化到托盘"
- **托盘图标右键菜单**：显示/隐藏窗口、新建待办、启动番茄钟、退出
- **双击托盘图标**：快速显示/隐藏窗口
- **关闭窗口时可选择**：
  - 询问我（默认）
  - 最小化到托盘
  - 直接退出应用

### 设置页面

- 左侧导航点击"⚙️ 设置"进入设置页面
- 番茄钟设置：调整工作时长、短休息、长休息、长休息间隔
- 外观设置：切换浅色/深色主题
- 系统设置：配置关闭窗口行为、是否最小化到托盘
- 快捷键设置：查看当前可用快捷键

## 开发计划

### v0.1
- 基础待办功能
- 子待办
- Markdown 内容

### v0.2
- 番茄钟基础计时（工作/短休息/长休息）
- 番茄钟暂停/继续/停止
- 系统通知（plyer）
- 番茄钟与待办关联
- 深色/浅色主题切换
- 优先级标记

### v0.3
- 番茄日历热力图（GitHub 风格）
- 点击日期查看当天番茄钟详情

### v0.4
- 标签系统（新建/编辑/删除标签，待办可关联标签）

### v0.5
- JSON 导入导出（备份/迁移数据）

### v0.6
- PyInstaller 打包（Windows/Linux 单文件 exe）
- GitHub Actions CI/CD（自动构建 Release）

### v0.7
- 待办事项归档功能（根待办及其子待办一起归档）
- 独立归档页面
- 只读查看模式（TodoViewer）
- 交互优化：单击只选中，双击查看，右键编辑
- 待办列表右侧复选框快速切换完成状态
- 刷新列表保持展开/选中状态
- 数据库迁移支持（status 字段）

### v0.8 (当前)
- 系统托盘图标（最小化到托盘，右键菜单）
- 关闭窗口行为选择（询问/最小化到托盘/直接退出）
- 记住关闭行为选项
- 完整设置页面（侧边栏分类导航）
  - 番茄钟设置（工作时长、短休息、长休息、长休息间隔）
  - 外观设置（浅色/深色主题切换）
  - 系统设置（关闭窗口行为、最小化到托盘）
  - 快捷键设置（显示当前快捷键）
- 设置持久化到数据库
- PyInstaller 打包资源路径修复（开发/打包环境兼容）
- 应用图标修复（窗口图标、任务栏图标、托盘图标统一）

## 快捷键

- `Ctrl+N` - 新建待办
- `Ctrl+P` - 打开/关闭番茄钟
- `Ctrl+Q` - 退出

## 数据导入导出

### 导出数据

1. 菜单栏点击「文件 → 导出...」
2. 选择保存位置
3. 点击「保存」

导出的 JSON 文件包含：
- 所有标签
- 所有待办事项（含子待办）
- 所有番茄钟记录
- 待办与标签的关联关系

### 导入数据

1. 菜单栏点击「文件 → 导入...」
2. 选择要导入的 JSON 文件
3. 选择导入方式：
   - **「是」** - 替换：删除所有现有数据后导入
   - **「否」** - 合并：保留现有数据，追加导入（可能重复）
4. 点击「确定」

导入后会显示导入的统计信息。

## 打包与发布

### Windows 本地打包

```powershell
# 在 PowerShell 中运行
.\build-windows.ps1
```

### Linux 本地打包

```bash
# 在 Bash 中运行
./build-linux.sh
```

### GitHub Actions CI/CD

**打标签并发布：**

```bash
# 打版本标签
git tag -a v0.7.0 -m "Release v0.7.0"
git push origin v0.7.0
```

GitHub Actions 会自动：
1. 在 Windows 和 Linux 上构建
2. 生成 `EsTodo-windows.zip` 和 `EsTodo-linux.tar.gz`
3. 创建 GitHub Release 并上传构建产物

**手动触发构建：**

也可以在 GitHub 仓库的 Actions 页面手动触发工作流。

### 下载使用

从 [Releases](https://github.com/MJYEEE/EsTodo/releases) 页面下载：

- **Windows**: 下载 `EsTodo-windows.zip`，解压后直接运行 `EsTodo.exe`
- **Linux**: 下载 `EsTodo-linux.tar.gz`，解压后运行 `EsTodo`

## 项目结构

```
EsTodo/
├── .github/
│   └── workflows/
│       └── build.yml          # GitHub Actions 构建配置
├── src/
│   └── estodo/
│       ├── __init__.py
│       ├── main.py              # 入口
│       ├── database.py          # 数据库管理
│       ├── import_export.py     # 导入导出功能
│       ├── models/              # 数据模型
│       │   ├── __init__.py
│       │   ├── todo.py
│       │   ├── pomodoro.py
│       │   └── tag.py
│       └── views/               # UI 组件
│           ├── __init__.py
│           ├── main_window.py
│           ├── todo_tree.py
│           ├── todo_editor.py
│           ├── todo_viewer.py    # 只读查看器
│           ├── settings_page.py   # 设置页面（侧边栏分类）
│           ├── pomodoro_timer.py
│           ├── heatmap.py
│           ├── day_detail_dialog.py
│           ├── tag_dialog.py
│           ├── tag_selector.py
│           ├── notifications.py
│           ├── theme.py
│           └── markdown.py
├── estodo.spec                  # PyInstaller spec 文件
├── build-windows.ps1           # Windows 打包脚本
├── build-linux.sh              # Linux 打包脚本
├── requirements.txt
├── README.md
└── run.sh
```

## License

MIT
